...
const texts = (names: string[]) =>
  cardLabels(names.map(label)).map((chip) => chip.text);

describe("parseLabel", () => {
  it("splits the namespace convention both repositories use", () => {
    expect(parseLabel(label("priority: P1"))).toMatchObject({
      namespace: "priority",
      value: "P1",
    });
    expect(parseLabel(label("effort: L"))).toMatchObject({
      namespace: "effort",
      value: "L",
    });
...
describe("cardLabels on a backlog that keeps the convention", () => {
  ...
  it("marks a slot the issue has no label for, rather than leaving it out", () => {
    ...
    expect(texts(["type: bug", "priority: P0"])).toEqual([
      "type: bug",
      "priority: P0",
      "effort",
    ]);
    expect(
      cardLabels([label("type: bug"), label("priority: P0")])[2],
    ).toMatchObject({
      namespace: "effort",
      empty: true,
    });
...
describe("cardLabels on a backlog that does not", () => {
  ...
  it("invents no gaps for a taxonomy the issue gives no evidence of", () => {
    ...
    expect(texts(["effort: M", "enhancement"])).toEqual([
      "effort: M",
      "enhancement",
    ]);
    expect(
      cardLabels([label("effort: M"), label("enhancement")]).every(
        (chip) => !chip.empty,
      ),
    ).toBe(true);
  ...
  it("keeps a namespace it recognizes in front of the ones it does not", () => {
    expect(
      texts(["Component: DevTools", "Type: Bug", "Status: Unconfirmed"]),
    ).toEqual(["Type: Bug", "Component: DevTools", "Status: Unconfirmed"]);
  });
  ...
  it("draws a label exactly as its repository spells it", () => {
    ...
    expect(texts(["Type:Bug", "priority: high"])).toEqual([
      "Type:Bug",
      "priority: high",
      "effort",
    ]);
  ...
  it("carries a long, punctuated or emoji label through untouched", () => {
    ...
    expect(texts([long, "🐛 bug", "C++ / stdlib"])).toEqual([
      long,
      "🐛 bug",
      "C++ / stdlib",
    ]);
  ...
  it("marks a namespace slot as canonical and every other chip as plain", () => {
    ...
    expect(chips.map((chip) => chip.namespace)).toEqual([
      "type",
      "priority",
      "effort",
      null,
    ...
    expect(
      cardLabels([label("enhancement")]).map((chip) => chip.namespace),
    ).toEqual([null]);
  });
});
