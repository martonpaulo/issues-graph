...
const painted = (selector: string) =>
  [
    resolve(declaration(selector, "color")),
    resolve(declaration(selector, "background")),
  ] as const;
...
function rgb(color: string): Rgb {
  ...
  if (!/^[0-9a-f]{6}$/i.test(hex))
    throw new Error(`not a six-digit hex colour: ${color}`);
  return [0, 2, 4].map((at) =>
...
function mix(
  ...
  const parsed = /color-mix\(in srgb,\s*(.+?)\s+(\d+)%,\s*transparent\)/.exec(
    declaration,
  );
  if (parsed === null)
    throw new Error(`unrecognised colour mix: ${declaration}`);
...
const pairs: ReadonlyArray<readonly [string, string, string]> = [
  ...
  [
    "the label picker while it is on",
...
describe("card text over every state fill", () => {
  ...
  it.each(stateFills)("%s carries all of its text at AA", (_state, fill) => {
    for (const [, alpha] of cardRoles) {
      expect(contrast(over(text, fill, alpha), fill)).toBeGreaterThanOrEqual(
        AA_NORMAL_TEXT,
      );
    }
  });
  ...
  it.each(stateFills)("%s carries both chip kinds at AA", (_state, fill) => {
    ...
    expect(
      contrast(over(text, fill, opacity(".chip--empty")), fill),
    ).toBeGreaterThanOrEqual(AA_NORMAL_TEXT);
  });

  it.each(stateFills)(
    "%s keeps the hovered card actions legible",
    (_state, fill) => {
      const button = mix(
        declaration(".card__actions .iconbutton", "background"),
        fill,
        resolve,
      );
      expect(contrast(token("muted"), button)).toBeGreaterThanOrEqual(
        AA_NORMAL_TEXT,
      );
    },
  );
...
describe("group labels", () => {
  ...
  it.each(frames)("%s labels its frame at AA", (frame, label) => {
    ...
    expect(contrast(declaration(label, "color"), fill)).toBeGreaterThanOrEqual(
      AA_NORMAL_TEXT,
    );
  });
  ...
  it("keeps the default label readable on a selected frame", () => {
    const fill = mix(
      declaration(".group--selected", "background"),
      token("bg"),
      resolve,
    );
    expect(contrast(token("muted"), fill)).toBeGreaterThanOrEqual(
      AA_NORMAL_TEXT,
    );
  });
});
