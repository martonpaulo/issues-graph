...
function blockersOf(raw: unknown): Map<number, IssuePayload[]> {
  return new Map(
    Object.entries(raw as Record<string, IssuePayload[]>).map(
...
async function share(
  ...
  Extract<Awaited<ReturnType<typeof buildSnapshotUrl>>, { kind: "ready" }>
> {
  ...
  if (link.kind !== "ready")
    throw new Error(`expected a link, got ${link.kind}`);
  return link;
}
...
describe("buildSnapshotUrl", () => {
  it("addresses the repository it was taken from", async () => {
    ...
    expect(
      link.url.startsWith(`${ORIGIN}${BASE}dependencies/martonpaulo/tabelo#g=`),
    ).toBe(true);
  });
  ...
  it("declines rather than truncating when the payload outgrows the budget", async () => {
    ...
    expect(link).toMatchObject({
      kind: "too-large",
      limit: SNAPSHOT_URL_LIMIT,
    });
    if (link.kind === "too-large")
      expect(link.chars).toBeGreaterThan(SNAPSHOT_URL_LIMIT);
  });
});
...
describe("readSnapshot", () => {
  ...
  it("sends the graph that was drawn, not the wider read behind it", async () => {
    ...
    const broad = graph(tabeloIssues, tabeloBlockedBy, {
      includedClosed: true,
    });
  ...
  it("reproduces the graph it was made from when a rename redirected the read", async () => {
    ...
    const redrawn = await buildGraph(read.view.data, {
      owner: "acme",
      repo: "app",
    });
    expect(redrawn.edges.map((edge) => edge.id)).toEqual(
      drawn.edges.map((edge) => edge.id),
    );
    ...
    const readOld = await readSnapshot(
      fragmentOf(misbound.url),
      "acme/old-app",
    );
    expect(readOld.kind).toBe("snapshot");
    if (readOld.kind !== "snapshot") return;

    const broken = await buildGraph(readOld.view.data, {
      owner: "acme",
      repo: "old-app",
    });
  ...
  it("rejects a link whose payload names a different repository", async () => {
    ...
    const read = await readSnapshot(
      fragmentOf(link.url),
      "martonpaulo/issues-graph",
    );

    expect(read).toEqual({
      kind: "invalid",
      reason:
        "This shared link holds martonpaulo/tabelo, not martonpaulo/issues-graph.",
    });
  });
  ...
  it("rejects a timestamp that would render as NaN rather than an age", async () => {
    ...
    for (const savedAt of bad) {
      const read = await readSnapshot(
        await fragmentFor(wholePayload({ savedAt })),
        "a/b",
      );
  ...
  it("rejects issues and blockers that are not the shape the cards read", async () => {
    ...
    for (const overrides of cases) {
      const read = await readSnapshot(
        await fragmentFor(wholePayload(overrides)),
        "a/b",
      );
  ...
  it("rejects a drawing that claims closed blockers its read never covered", async () => {
    ...
    expect(read).toEqual({
      kind: "invalid",
      reason: "This shared link contradicts itself.",
    });
  });
    ...
    const bytes = await new Response(
      new Blob([
        new TextEncoder().encode(
          JSON.stringify({ v: 2, slug: "a/b", graph: {}, shown: false }),
        ),
      ])
        .stream()
        .pipeThrough(new CompressionStream("deflate-raw")),
