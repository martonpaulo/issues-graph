...
/** The repository `withBrowserStorage` puts the page on. */
...
describe("nextIssueSelection", () => {
  it("replaces the selection for an ordinary issue click", () => {
    expect([...nextIssueSelection(new Set(["1", "2"]), "3", false)]).toEqual([
      "3",
    ]);
    expect([...nextIssueSelection(new Set(["1", "2"]), "2", false)]).toEqual([
      "2",
    ]);
  });

  it("toggles one issue for a modified click", () => {
    expect([...nextIssueSelection(new Set(["1"]), "2", true)]).toEqual([
      "1",
      "2",
    ]);
    expect([...nextIssueSelection(new Set(["1", "2"]), "2", true)]).toEqual([
      "1",
    ]);
  });
});
...
describe("saved copy entry", () => {
  ...
  it("describes a saved canvas with its age and dependency coverage", () => {
    ...
    expect(
      describeSavedCopy(
        { savedAt, includedClosed: false, source: "saved" },
        now,
      ),
    ).toBe("Saved copy · 2 hours ago · open blockers only");
    expect(
      describeSavedCopy(
        { savedAt, includedClosed: true, source: "saved" },
        now,
      ),
    ).toBe("Saved copy · 2 hours ago · includes closed blockers");
  });
  ...
  it("says a copy that arrived in a link is not the viewer\u2019s own", () => {
    ...
    expect(
      describeSavedCopy(
        { savedAt, includedClosed: false, source: "shared" },
        now,
      ),
    ).toBe("Shared copy · 2 hours ago · open blockers only");
    expect(
      describeSavedCopy(
        { savedAt, includedClosed: true, source: "shared" },
        now,
      ),
    ).toBe("Shared copy · 2 hours ago · includes closed blockers");
  });
});
...
describe("describeShare", () => {
  it("says what a copied link does for whoever receives it", () => {
    expect(
      describeShare({ kind: "copied", url: "https://example.test/#g=x" }),
    ).toBe(
  ...
  it("names both numbers when the graph is too large, and says nothing was shortened", () => {
    expect(
      describeShare({ kind: "too-large", chars: 41234, limit: 32000 }),
    ).toBe(
  ...
  it("states plainly when the browser cannot build one", () => {
    expect(describeShare({ kind: "unsupported" })).toBe(
      "This browser cannot build a shared link.",
    );
  });
});
...
describe("graphBounds", () => {
  ...
  it("encloses every card and every group frame of a captured graph", async () => {
    ...
    for (const group of graph.groups) {
      ...
      expect(group.position.y + group.height).toBeLessThanOrEqual(
        bounds.bottom,
      );
    }
...
describe("what the reader is told about the budget", () => {
  ...
  it("offers a token when the limit is hit without one, and does not when it is hit with one", () => {
    ...
    expect(failureText(target, failure, false).body).toContain(
      "Adding a token",
    );
    expect(failureText(target, failure, true).body).not.toContain(
      "Adding a token",
    );
    expect(failureText(target, failure, true).body).not.toContain(
      "Unauthenticated",
  ...
  it("blames the drawing, not GitHub, when the layout is what failed", () => {
    const text = failureText(
      target,
      { kind: "layout", message: "the worker stopped." },
      false,
    );
    ...
    const bodies = [
      ...
      failureText(
        target,
        {
          kind: "unexpected",
          status: 500,
          message: "GitHub returned error 500",
        },
        false,
      ),
...
describe("describeUnresolved", () => {
  ...
  it("ends the sentence once, whatever punctuation the reason arrived with", () => {
    const text = describeUnresolved([
      { number: 4, reason: "The request could not be sent." },
    ]);

    expect(text).toBe(
      "Some blocker data is missing: #4 — The request could not be sent.",
    );
  });

  it("says so plainly when the loader recorded no reason at all", () => {
    expect(describeUnresolved([])).toBe(
      "Some blocker data is missing, and GitHub did not say why.",
    );
  });
});
...
describe("the dependencies as text", () => {
  ...
  it("renders containment in its own table, which claims no ordering", async () => {
    ...
    // blocking #297 as well. A repository that does both is exactly what this table has to
    ...
    const html = renderToStaticMarkup(
      createElement(ContainmentTable, { rows }),
    );
...
describe("blockerStateText", () => {
  ...
  it("prefers the local workflow state where the repository shares that convention", () => {
    ...
    expect(
      blockerStateText(
        node({ state: "not-planned", open: false, external: false }),
      ),
    ).toBe("not planned");
  });
});
...
describe("the selection actions", () => {
  ...
  it("offers each action only where it would change something", () => {
    expect(bar({ canDim: true, canRestore: false })).not.toContain(
      "Restore the selected",
    );
    expect(bar({ canDim: false, canRestore: true })).not.toContain(
      "Dim the selected",
    );
  });
...
describe("what a key press asks of the canvas", () => {
  ...
  it("keeps the shortcuts the canvas has always had", () => {
    ...
    expect(canvasShortcut(press("a", { metaKey: true }), free, 0)).toBe(
      "select-all",
    );
    expect(canvasShortcut(press("A", { shiftKey: true }), free, 0)).toBe(
      "select-all",
    );
...
describe("where the focus ring is allowed to show", () => {
  ...
  it("shows the card icons whenever one of them takes the focus", () => {
    expect(styles).toMatch(
      /\.card__actions:focus-within[^{]*\{[^}]*opacity: 1/,
    );
  });
});
...
describe("what a local interaction rebuilds", () => {
  ...
  it("replaces only the selected card, and leaves every other node the object it was", async () => {
    ...
    const after = builder.nodes(
      graph,
      viewOf({ selected: new Set([picked]) }),
      handlers,
    );
    ...
    const changed = after
      .filter((node) => node !== was.get(node.id))
      .map((node) => node.id);
  ...
  it("leaves a card outside both highlights alone when the reader swaps one for the other", async () => {
    ...
    const handlers = handlersFor(graph);
    const labels = [
      ...new Set(graph.nodes.flatMap((node) => node.allLabels)),
    ].sort();
    expect(labels.length).toBeGreaterThan(1);
    const [first, second] = labels;

    const builder = createCanvasBuilder();
    const before = builder.nodes(
      graph,
      viewOf({ highlight: new Set([first]) }),
      handlers,
    );
    const after = builder.nodes(
      graph,
      viewOf({ highlight: new Set([second]) }),
      handlers,
    );
    ...
    const outside = graph.nodes.filter(
      (node) =>
        !node.allLabels.includes(first) && !node.allLabels.includes(second),
    );
    expect(outside.length).toBeGreaterThan(0);
    for (const node of outside) {
      expect(after.find((candidate) => candidate.id === node.id)).toBe(
        was.get(node.id),
      );
    }
  });
