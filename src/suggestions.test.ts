...
import { mergeSuggestions, recentTargets, rememberTarget } from "./suggestions";
...
describe("mergeSuggestions", () => {
  ...
  it("appends search results after the recents, without repeating one", () => {
    remember("a/one");
    expect(mergeSuggestions("one", ["a/one", "c/one"])).toEqual([
      "a/one",
      "c/one",
    ]);
  });
  ...
  it("does not offer a search result a recent already names in a different casing", () => {
    remember("Acme/App");
    expect(mergeSuggestions("app", ["acme/app", "other/app"])).toEqual([
      "Acme/App",
      "other/app",
    ]);
  });
