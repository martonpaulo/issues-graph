...
import { IssueCard, type IssueNode, STATE_TEXT } from "./IssueCard";
import { Icon } from "./icons";
import { dimmedKey, holdsData, releaseDimmed, saveDimmed } from "./retention";
import { reuse } from "./reuse";
import { canonicalSlugOf, type RepoTarget, slugOf } from "./route";
import {
  BASE,
  popupTriggerProps,
  SiteFooter,
  ...
  switch (failure.kind) {
    ...
    case "cancelled":
      return {
        ...
        body: "No dependency requests were spent.",
      };
    case "layout":
...
export function describeUnresolved(
  ...
  if (unresolved.length === 0)
    return "Some blocker data is missing, and GitHub did not say why.";
  ...
  const parts = [...groups.entries()].map(
    ([reason, numbers]) =>
      `${numbers.map((number) => `#${number}`).join(", ")} — ${reason}`,
  );
  ...
  return (
    <table className="deps__table">
      ...
      <caption className="deps__caption">
        {rows.length} dependenc{rows.length === 1 ? "y" : "ies"} ·{" "}
        {DIRECTION_LEGEND}
      </caption>
      <thead>
        <tr>
...
export function ContainmentTable({ rows }: { rows: ContainmentRow[] }) {
  return (
    <table className="deps__table">
      <caption className="deps__caption">
        {rows.length} sub-issue{rows.length === 1 ? "" : "s"} ·{" "}
        {CONTAINMENT_LEGEND}
      </caption>
      <thead>
        <tr>
...
export function relationshipSummary(
  ...
  const parts: string[] = [];
  if (dependencies > 0)
    parts.push(`${dependencies} dependenc${dependencies === 1 ? "y" : "ies"}`);
  if (containments > 0)
    parts.push(`${containments} sub-issue${containments === 1 ? "" : "s"}`);
  ...
  return (
    <span className="picker" data-open={open || undefined}>
      ...
      {open && (
        ...
        >
          <div className="picker__head">
            <span id={headingId}>Relationships</span>
            <button
              className="iconbutton"
              type="button"
...
export function graphBounds(graph: IssueGraph): GraphBounds {
  ...
  for (const node of graph.nodes) {
    ...
    if (node.position.x + NODE_WIDTH > right)
  ...
  for (const group of graph.groups) {
    ...
    if (group.position.x + group.width > right)
      right = group.position.x + group.width;
    if (group.position.y + group.height > bottom)
...
function useGraphLayout(graph: IssueGraph) {
  ...
  const translateExtent = useMemo<[[number, number], [number, number]]>(() => {
    const margin = Math.max(
      PAN_MARGIN_MIN,
      Math.max(bounds.width, bounds.height) * PAN_MARGIN_SHARE,
    );
    return [
...
export function canvasShortcut(
  ...
  if (event.key === "Enter" && selectionSize === 1 && !focus.onControl)
    return "open";
  return null;
}
...
function useCanvasShortcuts({
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      const shortcut = canvasShortcut(
        event,
        placeFocus(event.target),
        selected.size,
      );
  ...
  return (
    <div className="bar bar--identity">
      ...
      <a
        className="iconbutton"
        href={BASE}
        data-tip="Choose another repository"
      >
        <Icon name="graph" />
      </a>
...
function buildNodes(
  ...
  const cards: IssueNode[] = graph.nodes.map((node) => {
    const highlighted =
      highlight.size > 0 &&
      node.allLabels.some((label) => highlight.has(label));
    return {
      id: node.id,
      type: "issue" as const,
...
export function createCanvasBuilder(): CanvasBuilder {
  ...
  return {
    nodes: (graph, view, handlers) =>
      (nodes = reuse(nodes, buildNodes(graph, view, handlers))),
...
function Canvas({
  ...
  const [highlight, setHighlight] = useState<ReadonlySet<string>>(
    () => new Set(),
  );
...
export function GraphView({
  ...
  const [showClosed, setShowClosed] = useState(() =>
    readStored(SHOW_CLOSED_KEY, asBoolean, false),
  );
...
function GraphLoad({
  ...
  const { session, state } = useGraphSession({
    target,
    identity,
    token,
  ...
  const savedCopyDecision = cached
    ? decideSavedCopyOpen(cached, showClosed)
    : null;
    ...
    >
        ...
        <>
          ...
          {phase.status !== null && phase.status.remaining < phase.cost && (
            <p className="notice notice--error">
              More than the budget has left. The graph will come back
      ...
      {phase.kind === "failed" && (
        <>
          ...
          <p className="notice notice--error" role="status">
            <strong>
              {failureText(target, phase.failure, token !== "").title}.
            </strong>{" "}
