...
function dataFrom(
  ...
  const blockers = new Map(
    Object.entries(blockedBy as Record<string, IssuePayload[]>).map(
...
describe("chipRows and cardHeight", () => {
  it("wraps the chips a card cannot fit on one line", () => {
    ...
    expect(chipRows(["type: documentation", "priority: P1", "effort: M"])).toBe(
      2,
    );
  ...
  it("agrees with the browser on the combinations that wrap by a fraction of a pixel", () => {
    expect(chipRows(["type: performance", "priority: P1", "effort: L"])).toBe(
      2,
    );
    expect(chipRows(["type: documentation", "priority: P1", "effort"])).toBe(2);
    expect(chipRows(["type: improvement", "priority: P2", "effort: S"])).toBe(
      2,
    );
  ...
  it("reserves a second row for the chips that overflow the first, so nothing hangs out", () => {
    ...
    expect(cardHeight(1, chipRows(overflowing))).toBeGreaterThan(
      cardHeight(1, 1),
    );
  });
  ...
  it("over-reserves rather than under-reserves for text outside the captured latin subset", () => {
    ...
    expect(
      chipRows(["type: 改善改善改善改善改善改善改善改善改善改善改善改善"]),
    ).toBe(1);
  });
...
describe("repoOf", () => {
  it("reads owner/name out of an API repository URL", async () => {
    expect(repoOf("https://api.github.com/repos/martonpaulo/tabelo")).toBe(
      "martonpaulo/tabelo",
    );
  });
});
...
describe("deriveState", () => {
  it("uses the open-blocker count, not the total, to decide blocked", async () => {
    ...
    expect(deriveState(issue({ issue_dependencies_summary: summary }))).toBe(
      "ready",
    );
    expect(
      deriveState(
        issue({ issue_dependencies_summary: { ...summary, blocked_by: 1 } }),
      ),
    ).toBe("blocked");
  });
  ...
  it("separates a completed issue from one closed as not planned", async () => {
    expect(
      deriveState(issue({ state: "closed", state_reason: "completed" })),
    ).toBe("completed");
    expect(
      deriveState(issue({ state: "closed", state_reason: "not_planned" })),
    ).toBe("not-planned");
    expect(deriveState(issue({ state: "closed", state_reason: null }))).toBe(
      "completed",
    );
  });
  ...
  it("reads in-review as delivered rather than as available", async () => {
    ...
    expect(
      deriveState(
        issue({ labels: [{ name: "status: in-review", color: "8B949E" }] }),
      ),
    ).toBe("ready");
  });
  ...
  it("reads in-progress from the label itself, not from the status namespace", async () => {
    ...
    expect(
      deriveState(
        issue({ labels: [{ name: "status: blocked", color: "24292F" }] }),
      ),
    ).toBe("attention");
  });
  ...
  it("reads a foreign board column as the nothing it says", async () => {
    for (const name of [
      ...
      expect(
        deriveState(issue({ labels, assignees: [{ login: "octocat" }] })),
      ).toBe("ready");
    ...
    for (const name of ["status: blocked", "status: needs-decision"]) {
      expect(
        deriveState(
          issue({ labels: [{ name, color: "ededed" }], assignees: [] }),
        ),
      ).toBe("attention");
    }
  });
  ...
  it("separates an unassigned issue from one that is free to start", async () => {
    ...
    expect(deriveState(issue({ assignees: [{ login: "martonpaulo" }] }))).toBe(
      "ready",
    );
  });
  ...
  it("orders the open states so the fact that decides the issue wins", async () => {
    ...
    expect(
      deriveState(
        issue({
          labels: every,
          assignees: [],
          issue_dependencies_summary: blocked,
        }),
      ),
    ).toBe("in-review");
    expect(
    ...
    expect(
      deriveState(
        issue({ assignees: [], issue_dependencies_summary: blocked }),
      ),
    ).toBe("blocked");
  });
...
describe("a card built from a repository this viewer knows nothing about", () => {
  ...
  it("sizes itself for the chips it actually draws", async () => {
    const graph = await buildGraph(
      dataFrom([issue({ labels: arbitrary })], {}),
      {
        owner: "acme",
        repo: "app",
      },
  ...
  it("still reads state from GitHub's own facts when no label means anything here", () => {
    ...
    expect(
      deriveState(
        issue({ labels, state: "closed", state_reason: "completed" }),
      ),
    ).toBe("completed");
  });
});
...
describe("dependencyCounts", () => {
  it("counts ordering only, so a parent neither waits nor holds anything up", async () => {
    const graph = await buildGraph(
      dataFrom(tabeloIssues, tabeloBlockedBy),
      TABELO,
    );
...
describe("subIssuesOf", () => {
  it("reports a parent's progress and nothing for an issue that is not one", async () => {
    expect(
      subIssuesOf(
        issue({
          sub_issues_summary: { total: 5, completed: 2, percent_completed: 40 },
        }),
      ),
...
describe("parentNodeId", () => {
  it("reads the node a parent URL names, whatever repository it lives in", async () => {
    expect(
      parentNodeId(
        "https://api.github.com/repos/martonpaulo/tabelo/issues/294",
      ),
    ).toBe("martonpaulo/tabelo#294");
    expect(
      parentNodeId("https://api.github.com/repos/other/lib/issues/7"),
    ).toBe("other/lib#7");
  });
...
describe("buildGraph against captured GitHub data", () => {
  ...
  it("sizes every card to its own title and its own chips", async () => {
    ...
    for (const node of graph.nodes) {
      ...
      expect(node.height).toBe(
        cardHeight(
          node.titleLines,
          chipRows(node.labels.map((chip) => chip.text)),
        ),
      );
    }
  });
  ...
  it("draws the tabelo split as containment, alongside the ordering it also has", async () => {
    ...
    expect(
      hierarchy.every((edge) => edge.source === "martonpaulo/tabelo#294"),
    ).toBe(true);
    expect(
      graph.edges.some(
        (edge) =>
    ...
    expect(graph.nodes.filter((node) => node.subIssues !== null)).toHaveLength(
      1,
    );
  });
  ...
  it("reads every tabelo card as unassigned, because none of them is assigned", async () => {
    ...
    expect(new Set(graph.nodes.map((node) => node.state))).toEqual(
      new Set(["unassigned", "blocked"]),
    );
  });
  ...
  it("leaves a repository captured before these fields existed exactly as it was", async () => {
    ...
    expect(graph.groups.every((group) => group.kind !== "breakdown")).toBe(
      true,
    );
  });
  ...
  it.each([
    ...
    "draws no closed %s issue by default",
    async (_name, issues, blockedBy, target) => {
      const graph = await buildGraph(dataFrom(issues, blockedBy), target);
      expect(graph.nodes.some((node) => node.state === "completed")).toBe(
        false,
      );
      expect(graph.nodes.some((node) => node.state === "not-planned")).toBe(
        false,
      );
    },
  );
  ...
  it.each([
    ...
    async (_name, issues, blockedBy, target) => {
      ...
      const listed = new Set(
        (issues as IssuePayload[]).map((issue) => issue.number),
      );
      ...
      const recovered = graph.nodes.filter(
        (node) => !node.external && !listed.has(node.number),
      );
      expect(recovered.length).toBeGreaterThan(0);
      expect(
        recovered.every(
          (node) => node.state === "completed" || node.state === "not-planned",
        ),
      ).toBe(true);
    },
  );
  ...
  it.each([
    ...
    "frames every %s card in exactly one group",
    async (_name, issues, blockedBy, target) => {
      const graph = await buildGraph(dataFrom(issues, blockedBy), target);
      ...
      expect(
        graph.groups.filter((group) => group.kind === "free").length,
  ...
  it("derives blocked from GitHub rather than from the drawn edges", async () => {
    ...
    const byNumber = new Map(
      (arbaroIssues as IssuePayload[]).map((i) => [i.number, i]),
    );
    ...
    for (const node of graph.nodes) {
      ...
      expect(node.state === "blocked").toBe(
        openBlockers > 0 && node.state !== "attention",
      );
    }
  });
  ...
  it.each([
    ...
    "lays %s out in a shape a screen can show",
    async (_name, issues, blockedBy, target) => {
      const graph = await buildGraph(dataFrom(issues, blockedBy), target);
      ...
      const height =
        Math.max(...graph.nodes.map((node) => node.position.y + node.height)) -
        Math.min(...ys);
  ...
  it("lays nodes out without stacking them all at the origin", async () => {
    ...
    expect(
      Math.min(...graph.nodes.map((n) => n.position.x)),
    ).toBeGreaterThanOrEqual(0);
    expect(
      Math.min(...graph.nodes.map((n) => n.position.y)),
    ).toBeGreaterThanOrEqual(0);
  });
  ...
  it.each([
    ...
    "puts every %s blocker above what it blocks",
    async (_name, issues, blockedBy, target) => {
      const graph = await buildGraph(dataFrom(issues, blockedBy), target);
      const byId = new Map(graph.nodes.map((node) => [node.id, node]));
      ...
      for (const edge of graph.edges) {
        ...
        expect(byId.get(edge.source)!.position.y, edge.id).toBeLessThan(
          byId.get(edge.target)!.position.y,
        );
      }
    },
  );
  ...
  it("draws every arbaro dependency downwards, contradicted or not", async () => {
    ...
    const blockers = ["arbaro#37", "arbaro#38", "arbaro#39"].map(
      (id) => `martonpaulo/${id}`,
    );
    const parent = byId.get("martonpaulo/arbaro#18")!;
    for (const blocker of blockers) {
      expect(byId.get(blocker)!.position.y, blocker).toBeLessThan(
        parent.position.y,
      );
    }
  ...
  it("marks exactly the contradicted arbaro containment edges, and only those", async () => {
    ...
    expect(marked.map((edge) => edge.id).sort()).toEqual(
      [37, 38, 39].map(
        (child) => `martonpaulo/arbaro#18=>martonpaulo/arbaro#${child}`,
      ),
    );
    ...
    for (const edge of marked) {
      ...
      expect(byId.get(edge.source)!.position.y, edge.id).toBeGreaterThan(
        byId.get(edge.target)!.position.y,
      );
    }
  });
  ...
  it.each([
    ...
    "routes every %s edge orthogonally",
    async (_name, issues, blockedBy, target) => {
      const graph = await buildGraph(dataFrom(issues, blockedBy), target);
  ...
  it("gives each edge of a card its own point on it", async () => {
    ...
    for (const edge of graph.edges) {
      leaving.set(edge.source, [
        ...(leaving.get(edge.source) ?? []),
        edge.points![0].x,
      ]);
    }
  ...
  it("keeps each dependency chain inside its own packed block", async () => {
    ...
    for (const edge of graph.edges) {
      const dx = Math.abs(
        byId.get(edge.source)!.position.x - byId.get(edge.target)!.position.x,
      );
...
describe("buildGraph edge cases not present in the captured data", () => {
  it("adds a blocker from another repository as an external node", async () => {
    ...
    const graph = await buildGraph(dataFrom([local], { 5: [foreign] }), {
      owner: "acme",
      repo: "app",
    });
  ...
  it("drops the owner from a blocker that shares it with the repository being viewed", async () => {
    ...
    const graph = await buildGraph(dataFrom([local], { 5: [sibling] }), {
      owner: "acme",
      repo: "app",
    });
  ...
  it("ignores a dependency whose dependent issue is not in the list", async () => {
    const graph = await buildGraph(
      dataFrom([issue({ number: 1 })], { 99: [issue({ number: 2 })] }),
      {
        owner: "acme",
        repo: "app",
      },
  ...
  it("drops a closed blocker and the edge into it", async () => {
    ...
    const withClosed = await buildGraph(
      data,
      { owner: "acme", repo: "app" },
      { showClosed: true },
    );
  ...
  it("deduplicates a blocker listed twice", async () => {
    ...
    const graph = await buildGraph(
      dataFrom([blocked, issue({ number: 2 })], {
        5: [issue({ number: 2 }), issue({ number: 2 })],
      }),
      { owner: "acme", repo: "app" },
    );
  ...
  it("draws a sub-issue as containment, from the child, at no request cost", async () => {
    ...
    const graph = await buildGraph(dataFrom([parent, child], {}), {
      owner: "acme",
      repo: "app",
    });
  ...
  it("frames a containment-only set as a breakdown rather than as a chain", async () => {
    ...
    expect(graph.groups).toMatchObject([
      { kind: "breakdown", label: "Breakdown · 2 issues" },
    ]);
  });
  ...
  it("drops a parent that lives in another repository instead of fetching it", async () => {
    ...
    const graph = await buildGraph(dataFrom([child], {}), {
      owner: "acme",
      repo: "app",
    });
    expect(graph.nodes).toHaveLength(1);
...
describe("repository identity is canonical, not what the address happened to spell", () => {
  ...
  it("draws the same graph from a mixed-case route as from the canonical one", async () => {
    ...
    const mixed = await buildGraph(blockedPair(), {
      owner: "Acme",
      repo: "App",
    });
    const exact = await buildGraph(blockedPair(), {
      owner: "acme",
      repo: "app",
    });
  ...
  it("keeps local issues local when the address spells the repository differently", async () => {
    const graph = await buildGraph(blockedPair(), {
      owner: "ACME",
      repo: "APP",
    });
  ...
  it("reports the identity it drew, so what leaves this browser is bound to it", async () => {
    ...
    expect(
      redirected.nodes.every((node) => node.id.startsWith("acme/app#")),
    ).toBe(true);

    // Untrusted data does not get to name it, so the identity is the address, canonically.
    const untrusted = await buildGraph(blockedPair(), {
      owner: "Acme",
      repo: "App",
    });
  ...
  it("still qualifies a cross-repository blocker by its own repository", async () => {
    ...
    const graph = await buildGraph(
      dataFrom([blocked], { 5: [sibling, foreign] }),
      {
        owner: "Acme",
        repo: "App",
      },
    ...
    expect(byNumber.get(7)).toMatchObject({
      id: "acme/other#7",
      external: true,
      repoLabel: "other",
    });
