...
function installQuotaStorage(limit: number): Map<string, string> {
  ...
  const storage = {
    ...
    setItem: (key: string, value: string) => {
      ...
      const after =
        used() -
        (existing === undefined ? 0 : key.length + existing.length) +
        key.length +
        value.length;
      if (after > limit)
...
describe("a browser already at its quota before the index existed", () => {
  ...
  it("leaves the index naming only repositories whose data is really there", () => {
    ...
    for (const entry of retained()) {
      ...
      expect(
        held,
        `${entry.slug} is named by the index but holds nothing`,
      ).toBe(true);
    }
  });
...
describe("a refused index write destroys nothing", () => {
  ...
  it("does not leave the index claiming data it just deleted", () => {
    ...
    for (const entry of retained()) {
      const identity = canonicalSlug(entry.slug);
      const present =
        store.has(cacheKey(identity)) || store.has(dimmedKey(identity));
      expect(
        present,
        `${entry.slug} is named by the index but holds nothing`,
      ).toBe(true);
...
describe("saveDimmed holds its two halves together", () => {
  ...
  it("leaves nothing behind when the repository cannot be indexed", () => {
    ...
    expect(saveDimmed("shared/link", ["issue-1"])).toMatchObject({
      ok: false,
      reason: "quota",
    });
    expect(store.has(dimmedKey("shared/link"))).toBe(false);
    ...
    installStorage(
      {
        setItem: (key: string) => {
          ...
          throw new Error(
            "the index must not be written when the value was not",
          );
        },
      },
      store,
  ...
  it("puts back the cards that were already there", () => {
    ...
    expect(store.get(dimmedKey("shared/link"))).toBe(
...
describe("the size budget", () => {
  ...
  it("counts the size of the copy that is actually held, not the number of visits", () => {
    ...
    expect(retained()).toEqual([
      expect.objectContaining({ slug: "o/one", chars: 20 }),
    ]);
  });
});
...
describe("touchRepository", () => {
  it("moves a repository back to the front without touching its size or its spelling", () => {
    ...
    expect(retained()[0]).toEqual(
      expect.objectContaining({ slug: "Acme/App", chars: 42 }),
    );
  });
});
