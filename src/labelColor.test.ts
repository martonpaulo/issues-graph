...
const rgb = (hex: string) =>
  [0, 2, 4].map((at) =>
    Number.parseInt(hex.slice(1).slice(at, at + 2), 16),
...
describe("contrast arithmetic", () => {
  it("agrees with the two anchors WCAG defines", () => {
    expect(
      Math.round(contrast(rgb("#000000"), rgb("#ffffff")) * 100) / 100,
    ).toBe(21);
    expect(
      Math.round(contrast(rgb("#777777"), rgb("#ffffff")) * 100) / 100,
    ).toBe(4.48);
  });
});
...
describe("chipPalette", () => {
  ...
  it.each([
    ...
    "00ffff",
    "ffffff",
  ])("holds the sRGB corner %s at AA", (color) => {
  ...
  it("keeps the background light and the foreground dark, whatever the label is", () => {
    ...
    for (const color of ["000000", "ffffff", "d73a4a", "0075ca"]) {
      const palette = chipPalette(color)!;
      expect(
        ratio({ foreground: palette.background, background: "#ffffff" }),
      ).toBeLessThan(2);
      expect(
        ratio({ foreground: palette.foreground, background: "#ffffff" }),
      ).toBeGreaterThan(4);
    }
  });
