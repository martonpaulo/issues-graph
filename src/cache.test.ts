...
describe("readCache", () => {
  ...
  it("reads back the graph a write saved", () => {
    writeCache(
      "martonpaulo/tabelo",
      graph({ complete: false, includedClosed: true }),
    );
    ...
    expect([...(cached?.data.blockers.keys() ?? [])]).toEqual([
      ...blockers.keys(),
    ]);
    expect(
      cached?.data.blockers
        .get([...blockers.keys()][0])
  ...
  it("carries the unresolved dependencies that made a graph incomplete", () => {
    writeCache(
      ...
      graph({
        complete: false,
        unresolved: [{ number: 49, reason: "network" }],
      }),
    );
  ...
  it("ignores a copy whose summary count is a container rather than a number", () => {
    corrupt((stored) => {
      (
        stored.issues as Record<string, unknown>[]
  ...
  it("reads back an issue that carries no summary at all", () => {
    corrupt((stored) => {
      delete (stored.issues as Record<string, unknown>[])[0]
        .issue_dependencies_summary;
    });

    expect(
      readCache("martonpaulo/tabelo")?.data.issues[0]
        .issue_dependencies_summary,
    ).toBeUndefined();
  });
...
describe("writeCache", () => {
  it("stores only the fields the graph consumes, so the quota holds", () => {
    ...
    expect(Object.keys(stored.issues[0].labels[0]).sort()).toEqual([
      "color",
      "name",
    ]);
    ...
    const parent = stored.issues.find(
      (issue: { number: number }) => issue.number === 294,
    );
    expect(parent.sub_issues_summary).toEqual({
      total: 2,
      completed: 0,
      percent_completed: 0,
    });
    const child = stored.issues.find(
      (issue: { number: number }) => issue.number === 296,
    );
    expect(child.parent_issue_url).toBe(
  ...
  it("round-trips a copy written before assignees and sub-issues were read", () => {
    ...
    writeCache(
      "martonpaulo/tabelo",
      graph({ issues: older, blockers: new Map() }),
    );
  ...
  it("drops a payload field the graph does not read", () => {
    const withExtra = [
      { ...issues[0], pull_request: { url: "https://example.invalid" } },
    ];
    writeCache(
      "martonpaulo/tabelo",
  ...
  it("replaces the previous copy of the same repository", () => {
    ...
    writeCache(
      "martonpaulo/tabelo",
      graph({ issues: [issues[0]], blockers: new Map() }),
    );
  ...
  it("records the size of the copy it saved, so the budget has something to measure", () => {
    ...
    expect(held.chars).toBe(
      entries.get(cacheKey("martonpaulo/tabelo"))?.length,
    );
  });
  ...
  it("gives up rather than spinning when the index cannot record an eviction", () => {
    ...
    expect(writeCache("martonpaulo/tabelo", graph())).toMatchObject({
      reason: "quota",
    });
  });
  ...
  it("does not report a copy as saved when the index would not record it", () => {
    ...
    expect(writeCache("o/fresh", graph())).toMatchObject({
      ok: false,
      reason: "quota",
    });
  });
