...
import { type CachedGraph, readCache, writeCache } from "./cache";
import {
  type LoadFailure,
  type LoadResult,
  loadRepositoryGraph,
 ...
 */
export function stopsForTokenChange(
  kind: Phase["kind"],
...
export class GraphSession {
      ...
      .loadRepositoryGraph(this.#options.target, {
        ...
        onProgress: ({ done, total }) => {
          if (this.#current(run, controller))
            this.#set({ phase: { kind: "resolving", done, total } });
        },
        confirmDependencies: (cost) => this.#askToSpend(cost, controller),
      })
      .then((result) =>
        this.#drawLoaded(result, includeClosed, run, controller),
      )
      .catch((error: unknown) => {
        ...
        this.#set({
          phase: {
            ...
            failure: {
              kind: "network",
              message: messageOf(error, "The request failed."),
            },
          },
        });
      });
  ...
  openSavedCopy(showClosed: boolean): void {
    ...
    void this.#effects
      .buildGraph(copy.data, this.#options.target, {
        showClosed,
      ...
      .catch((error: unknown) => {
        if (!this.#current(run, controller)) return;
        this.#set({
          phase: { kind: "failed", failure: this.#layoutFailure(error) },
        });
      });
  }
  ...
  close(): void {
    ...
    if (this.#state.phase.kind === "drawing")
      this.#effects.discardLayoutEngine();
    this.#cancelWork();
  }
  ...
  #layoutFailure(error: unknown): SessionFailure {
    return {
      kind: "layout",
      message: messageOf(error, "The layout could not be computed."),
    };
  }
      ...
      .then((status) => {
        ...
        return new Promise<boolean>((resolve) => {
          ...
          const settle = (approved: boolean) => {
            ...
            if (this.#settleConfirmation === settle)
              this.#settleConfirmation = null;
            resolve(approved);
          };
          ...
          this.#set({
            phase: { kind: "confirm", cost, status, decide: settle },
          });
        });
      });
  }
    ...
    void this.#effects
      ...
      .then(async (read) => {
        ...
        try {
          ...
          if (isCurrent())
            this.#abandonLink(
              "This shared link was read, but could not be drawn.",
            );
          return;
        }
      ...
      .catch(() => {
        if (isCurrent())
          this.#abandonLink("This shared link could not be read.");
      });
  }
