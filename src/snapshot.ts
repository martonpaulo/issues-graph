...
import { fromStored, isStoredGraph, type StoredGraph, toStored } from "./cache";
...
export async function readSnapshot(
  ...
  if (!supported()) {
    return {
      kind: "invalid",
      reason: "This browser cannot read shared links.",
    };
  }
  ...
  if (
    ...
    canonicalSlug(payload.slug) !== canonicalSlug(slug)
  ) {
    return {
      kind: "invalid",
