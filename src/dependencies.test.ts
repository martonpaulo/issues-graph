...
describe("adjacencyOf", () => {
  it("reads both directions of every drawn edge and nothing else", async () => {
    ...
    for (const [id, entry] of adjacency) {
      for (const dependent of entry.blocks)
        mirrored.push(`${id}->${dependent.id}`);
    }
  ...
  it("follows the drawn edges rather than GitHub’s own dependency counts", async () => {
    ...
    const target = (arbaroIssues as IssuePayload[]).find(
      (candidate) => candidate.number === 11,
    );
    expect(
      target?.issue_dependencies_summary?.total_blocked_by,
    ).toBeGreaterThan(0);
    ...
    expect(describe_(open, nodeId("martonpaulo/arbaro", 11))).toContain(
      "Blocked by nothing.",
    );

    const closed = await buildGraph(
      dataFrom(arbaroIssues, arbaroBlockedBy),
      ARBARO,
      {
        showClosed: true,
    ...
    expect(describe_(closed, nodeId("martonpaulo/arbaro", 11))).toContain(
      "Blocked by #1 and #5.",
    );
  });
});
...
describe("describeNode", () => {
  it("states a fork as one sentence per direction", async () => {
    ...
    expect(describe_(graph, "acme/app#3")).toBe(
      "Issue #3. Blocked by #1. Blocks #4.",
    );
    expect(describe_(graph, "acme/app#4")).toBe(
      "Issue #4. Blocked by #3. Blocks nothing.",
    );
  });

  it("keeps a cross-repository blocker qualified on both cards", async () => {
    const graph = await buildGraph(
      dataFrom([issue({ number: 5 })], {
        5: [
          issue({
            number: 9,
            repository_url: "https://api.github.com/repos/other/lib",
...
describe("a sub-issue hierarchy, as GitHub actually returned one", () => {
  ...
  it("says every drawn hierarchy edge, and says it as containment", async () => {
    ...
    expect(
      containmentRows(graph)
        .map((row) => row.id)
        .sort(),
    ).toEqual(drawn.map((edge) => edge.id).sort());
    ...
    for (const edge of drawn) {
      ...
      expect(describe_(graph, child.id)).toContain(
        `Part of ${issueRef(parent)}`,
      );
    }
  });
...
describe("a sub-issue hierarchy, at the IssueGraph seam", () => {
  function node(
    over: Partial<GraphNode> & Pick<GraphNode, "id" | "number" | "repo">,
  ): GraphNode {
    return {
  ...
  it("keeps a sub-issue whose parent is elsewhere qualified on both cards", () => {
    ...
    expect(
      containmentRows(graph).map(
        (row) => `${issueRef(row.parent)}->${issueRef(row.child)}`,
      ),
  ...
  it("never lets containment reach an ordering, in either direction", () => {
    ...
    expect(adjacency.get("acme/app#5")?.children.map((n) => n.id)).toEqual([
      "acme/app#6",
    ]);
    // And the real blocker is nobody's parent.
    expect(adjacency.get("acme/app#7")?.children).toEqual([]);
    expect(adjacency.get("acme/app#6")?.parents.map((n) => n.id)).toEqual([
      "acme/app#5",
    ]);
    ...
    expect(describe_(graph, "acme/app#7")).toBe(
      "Issue #7. Blocked by nothing. Blocks #6.",
    );

    // The two tables partition the edges: neither holds one of the other's.
    expect(dependencyRows(graph).map((row) => row.id)).toEqual([
      "acme/app#7->acme/app#6",
    ]);
    expect(containmentRows(graph).map((row) => row.id)).toEqual([
  ...
  it("reports a closed parent that the drawing chose to include", () => {
    ...
    expect(containmentRows(graph).map((row) => row.parent.open)).toEqual([
      false,
    ]);
  });
  ...
  it("says nothing at all when the graph drew no containment", () => {
    ...
    expect(describe_(graph, "acme/app#5")).toBe(
      "Issue #5. Blocked by nothing. Blocks #6.",
    );
    expect(describe_(graph, "acme/app#6")).toBe(
      "Issue #6. Blocked by #5. Blocks nothing.",
    );
  });
...
describe("a closed blocker in another repository", () => {
  ...
  it("carries GitHub’s own closed state, which the local presentation state withholds", async () => {
    ...
    expect(dependencyRows(graph).map((row) => row.blocker.id)).toEqual([
      "other/lib#9",
    ]);
  });
...
describe("dependencyRows", () => {
  it("covers every drawn edge exactly once, in a stable order", async () => {
    ...
    expect(
      rows.map((row) => `${row.blocker.id}->${row.dependent.id}`).sort(),
    ).toEqual(drawn.map((edge) => edge.id).sort());
    ...
    const ref = (node: { repo: string; number: number }) =>
      [node.repo, node.number] as const;
    const order = rows.map(
      (row) => [...ref(row.blocker), ...ref(row.dependent)] as const,
    );
  ...
  it("grows with the edges the closed-blocker view adds", async () => {
    const open = dependencyRows(
      await buildGraph(dataFrom(arbaroIssues, arbaroBlockedBy), ARBARO),
    );
