...
describe("what a submit opens", () => {
  ...
  function highlightSecond(): number {
    ...
    active =
      nextActiveOption("ArrowDown", active, suggestions.length) ?? active;
  ...
  it("opens the typed text once Escape has given the highlight up", () => {
    const active =
      nextActiveOption("Escape", highlightSecond(), suggestions.length) ?? -1;
    expect(active).toBe(-1);
...
describe("SuggestionList", () => {
  it("gives each suggestion one option semantic with no interactive descendant", () => {
    ...
    expect(markup).toContain(
      '<li id="repo-0" role="option" aria-selected="false"',
    );
    expect(markup).toContain(
      '<li id="repo-1" role="option" aria-selected="false"',
    );
  ...
  it("marks the highlighted option, and only that one, as selected", () => {
    ...
    expect(markup).toContain(
      '<li id="repo-1" role="option" aria-selected="true"',
    );
